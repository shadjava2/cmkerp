...
/**
 * Command Service pour la gestion des transferts internes (écriture uniquement) - module POS.
 */
@Service("posTransfertInterneCommandService")
@Transactional
@Slf4j
public class TransfertInterneCommandService {
  ...
  public TransfertInterneResponse create(CreateTransfertInterneRequest request, Long currentUserId) {
    ...
    if (request.getLignes() != null && !request.getLignes().isEmpty()) {
      Map<Long, Float> reservedByStock = new HashMap<>();
      for (CreateLigneTransfertInterneRequest ligne : request.getLignes()) {
        ...
        stockValidator.verifyStockExists(ligne.getFkStock());
        stockValidator.verifyStockNotExpired(ligne.getFkStock());
        float alreadyReserved = reservedByStock.getOrDefault(ligne.getFkStock(), 0f);
        stockValidator.verifyStockAvailable(
            ligne.getFkStock(), ligne.getQuantite(), request.getFkPharmacieSource(), null, null, alreadyReserved);
        reservedByStock.merge(ligne.getFkStock(), ligne.getQuantite(), Float::sum);
      }
    }
  ...
  private void updateStocksOnValidation(TransfertInterne transfert, List<LigneTransfertInterne> lignes,
    ...
    for (LigneTransfertInterne ligne : lignes) {
      ...
      stockValidator.verifyStockAvailable(ligne.getFkStock(), ligne.getQuantite(), fkPharmacieSource);
