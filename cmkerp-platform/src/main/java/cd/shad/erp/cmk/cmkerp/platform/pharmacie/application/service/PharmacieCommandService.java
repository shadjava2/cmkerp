...
/**
 * Command Service pour la gestion des pharmacies (écriture uniquement).
 *
 * <p>
 * Ce service contient toutes les opérations de modification (commands) liées aux pharmacies. Toutes
...
@Transactional
@Slf4j
public class PharmacieCommandService {
  ...
  public PharmacieResponse create(PharmacieRequest request, Long currentUserId) {
    ...
    Pharmacie pharmacie = Pharmacie.builder().fkSite(request.getFkSite())
        .designation(request.getDesignation()).typePharmacie(request.getTypePharmacie())
        .codeimmo(request.getCodeimmo()).typeHospi(request.getTypeHospi())
    ...
    Pharmacie created = request.getCodeimmo() != null && !request.getCodeimmo().isEmpty()
        ? pharmacieRepository.findByCodeImmo(request.getCodeimmo()).orElseThrow(
            () -> new BusinessException("Erreur lors de la récupération de la pharmacie créée"))
        : pharmacieRepository.findBySite(request.getFkSite()).stream()
            .filter(p -> p.getDesignation().equals(request.getDesignation())).findFirst()
            .orElseThrow(() -> new BusinessException(
                "Erreur lors de la récupération de la pharmacie créée"));

    log.info("Pharmacie créée avec succès: ID={}, designation={}", created.getId(),
  ...
  public void deleteById(Long id) {
    ...
    pharmacieRepository.findById(id).orElseThrow(() -> NotFoundException.entity("Pharmacie", id));
  ...
  private PharmacieResponse pharmacieToResponseWithSite(Pharmacie pharmacie) {
    ...
    PharmacieResponse response = PharmacieResponse.builder().id(pharmacie.getId())
        .fkSite(pharmacie.getFkSite()).designation(pharmacie.getDesignation())
        .typePharmacie(pharmacie.getTypePharmacie()).codeimmo(pharmacie.getCodeimmo())
        .typeHospi(pharmacie.getTypeHospi()).dateCreate(pharmacie.getDateCreate())
        .dateUpdate(pharmacie.getDateUpdate()).userCreatedId(pharmacie.getUserCreatedId())
        .userUpdatedId(pharmacie.getUserUpdatedId()).build();
